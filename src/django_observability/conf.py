"""Configuration module for Django Observability.
...
def get_config() -> dict[str, Any]:
    """
    Get the Django Observability configuration from Django settings.

    Returns a dictionary with all configuration merged from:
    1. Default values
    2. DJANGO_OBSERVABILITY settings dict
    ...
    defaults = {
        "SERVICE_NAME": os.getenv(
            "OTEL_SERVICE_NAME",
            os.getenv("DJANGO_OBSERVABILITY_SERVICE_NAME", "django-app"),
        ),
        ...
        "CUSTOM_TAGS": {},
        "TRACING": {
            "ENABLED": _get_bool_env("DJANGO_OBSERVABILITY_TRACING_ENABLED", True),
            "OTLP_ENDPOINT": os.getenv(
                "OTEL_EXPORTER_OTLP_ENDPOINT", "http://localhost:4317"
            ),
            "SAMPLE_RATE": float(os.getenv("OTEL_TRACES_SAMPLER_ARG", "1.0")),
            "CONSOLE_EXPORTER": _get_bool_env(
                "DJANGO_OBSERVABILITY_CONSOLE_EXPORTER", False
            ),
        },
        "LOGGING": {
            "ENABLED": _get_bool_env("DJANGO_OBSERVABILITY_LOGGING_ENABLED", True),
            "FORMAT": os.getenv(
            ...
            "OTLP_ENABLED": _get_bool_env(
                "DJANGO_OBSERVABILITY_LOG_OTLP_ENABLED", True
            ),
            "OTLP_ENDPOINT": os.getenv(
                "OTEL_EXPORTER_OTLP_ENDPOINT", "http://localhost:4317"
            ),
        ...
        "METRICS": {
            "PROMETHEUS_ENABLED": _get_bool_env(
                "DJANGO_OBSERVABILITY_PROMETHEUS_ENABLED", True
            ),
        ...
        "CELERY": {
            ...
            "TRACING_ENABLED": _get_bool_env(
                "DJANGO_OBSERVABILITY_CELERY_TRACING_ENABLED", True
            ),
            "LOGGING_ENABLED": _get_bool_env(
                "DJANGO_OBSERVABILITY_CELERY_LOGGING_ENABLED", True
            ),
            "METRICS_ENABLED": _get_bool_env(
                "DJANGO_OBSERVABILITY_CELERY_METRICS_ENABLED", True
            ),
        },
        "PROFILING": {
            "ENABLED": _get_bool_env("DJANGO_OBSERVABILITY_PROFILING_ENABLED", False),
    ...
    user_config = getattr(settings, "DJANGO_OBSERVABILITY", {})
...
def get_observability_config() -> dict[str, Any]:
    """Get the global observability configuration (cached via lru_cache)."""
    return get_config()
