...
def test_observability_command_exists():
    from django_observability.management.commands.observability import Command

    assert Command is not None


def test_observability_command_help():
    from django_observability.management.commands.observability import Command

    cmd = Command()
    assert cmd.help is not None
    assert "observability" in cmd.help.lower()


def test_check_docker_compose_available():
    from django_observability.management.commands.observability import (
...
def test_get_compose_cmd_real():
    from django_observability.management.commands.observability import _get_compose_cmd
...
def test_get_work_dir_creates_directory():
    from django_observability.management.commands.observability import _get_work_dir
    ...
    assert str(work_dir).endswith(".django-observability")
...
def test_check_packages_real():
    from django_observability.management.commands.observability import _check_packages

    ok, warn, err = _check_packages()
...
def test_stack_start_dry_run():
    from django_observability.management.commands.observability import start
...
def test_stack_start_output(observability_stack):
    ...
    assert "observability stack stop" in result.output
...
def test_django_call_command_check(observability_stack):
    ...
    call_command("observability", "check", stdout=out)
